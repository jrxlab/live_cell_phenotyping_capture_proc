...
<span style="color:red">Insert Figure_dr1_v3.pdf</span> 
...
<span style="color:red">insert crop Figure_dr1_v3.pdf</span> 
...
<span style="color:red">insert fig D  </span> 
...
<span style="color:red">--> Import DVtoZeiss  </span> 

  

<span style="color:red">--> Run DVtoZeiss.allConversion(  

                  path_to_dv_pts_file=" folder_file_DV_dataset_path ",  

                  name_dv_pts_file="name_file_DV_fields_coordinates.pts",  

                  path_saved_zeiss_file="path_of_the_folder_where_to_save_Zeiss_new_coordinates",  

                  name_saved_zeiss_file="name_Zeiss_coordinates_file_to_be_created")  

  

  

If you have the file with DV coordinates for the 20 fields + a numpy array with the DV coordinates of the maker points (DV_array _maker_points) and the numpy array of the corresponding coordinates in the Zeiss (Zeiss_array_maker_points)  
...
<span style="color:red">--> Run DVtoZeiss.allConversion(  Zeiss_init= Zeiss_array_maker_points,  

                  DV_init= DV_array _maker_points) ,  
...
3 - <span style="color:red">Conversion vector B such that: </span> 
...
<=>   
         ...
         $X_{Zeiss}=A*X_{DV}+B$ with $X_{Zeiss}=\begin{pmatrix}x_{Zeiss} \\ y_{Zeiss}\end{pmatrix}, X_{DV}=\begin{pmatrix}x_{DV} \\ y_{DV}\end{pmatrix}       

        and $A=\begin{pmatrix}0 &c \\a&0\end{pmatrix}$ and $B=\begin{pmatrix}b \\d\end{pmatrix}$ 
...
Define the markers point coordinates in the DeltaVision (DV_ARRAY) and the corresponding coordinates in Zeiss (ZEISS_ARRAY)  

  

Define path (PATH_TO_DV_PTS_FILE) toward the .pts file with the 20 fields coordinates on the DeltaVision and the name of the file (NAME_DV_PTS_FILE)  

  

Define path (PATH_SAVED_ZEISS_FILE) toward the output .csv file with the 20 fields coordinates in the DeltaVision and their converted coordinates in Zeiss and the name of this .csv (NAME_SAVED_ZEISS_FILE)  
...
<li>computeLinearCoefficientDVtoZeiss(DV=DV_ARRAY, Zeiss=ZEISS_ARRAY) </li> </ul> 

  

<ul> 

<li>computeZeissCoordinatesFromDVcoordinates(DV=DV_ARRAY,A=[],B=[],Zeiss_init=[],DV_init=[]) </li></ul> 

  

<ul> 

<li>openDVptsFile(pathto_pts_DV_file) </li></ul> 

  

<ul> 

<li>plotFrameSlide(coordinate_array,ax=[]) </li></ul> 

  

<ul> 

<li>plotCoordinatesConversion(DV_array,Zeiss_coordinates_array,title='') </li></ul> 

  

<ul> 

<li>allConversion(A=[],B=[],Zeiss_init=[],DV_init=[],  
                  ...
                  name_saved_zeiss_file=NAME_SAVED_ZEISS_FILE) </li> </ul> 
